...
% Note: **** Here we are considering only lagging cells
...
%offset = 1.5;
offset = 5;
% Initialize vector to hold noise threshold values after scaling with offset
threshold_cfar = zeros(Ns-(G+T+1),1);
%threshold_cfar = zeros(Ns-(G+T),1);

% Initialize Vector to hold final signal after thresholding
signal_cfar = zeros(Ns-(G+T+1),1);
%signal_cfar = zeros(Ns-(G+T),1);
